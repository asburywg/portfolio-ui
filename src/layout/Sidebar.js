...
export function SidebarItem({ icon: Icon, text, alert, disabled }) {
    const { expanded, activeItem, setActiveItem } = useContext(SidebarContext);

    return (
        <li className={`relative flex items-center py-2 px-3 my-1 font-medium rounded-md  transition-colors group
                        ${disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer"} 
                        ${activeItem === text ? "bg-gradient-to-tr from-indigo-200 to-indigo-100 text-indigo-800" : "hover:bg-indigo-50 text-gray-600"}`}
            onClick={() => !disabled && setActiveItem(text)}>
            <Icon size={20} />
